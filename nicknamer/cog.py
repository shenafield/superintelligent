...
class NicknamerCog(commands.Cog):
    ...
    async def on_message(self, message: discord.Message):
        ...
        if message == '+rename':
            nickname, explaination = self.nicknamer.comeupwith(chat)
            await message.author.edit(nick=nickname)
            await message.channel.send(f"I chose {message.author.mention} a new nickname - {nickname} because they're lazy and asked me to choose one for them.\nI chose this nickname because {explaination}.\n\nGimme money.")
